...
class LSTMAutoencoder(object):
  ...
  def __init__(self, hidden_num, inputs, 
    seq_len=None, cell=None, optimizer=None,
    reverse=True, decode_without_input=True):
    """
    Args:
    ...
    with tf.variable_scope('encoder'):
      self.z_codes, self.enc_state = tf.nn.dynamic_rnn(
        self._enc_cell, inputs, sequence_length=seq_len,dtype=tf.float32)
    ...
    with tf.variable_scope('decoder') as vs:
      ...
      if decode_without_input:
        ...
        dec_outputs, dec_state = tf.nn.rnn(
          ...
          initial_state=self.enc_state, sequence_length=seq_len, dtype=tf.float32)
